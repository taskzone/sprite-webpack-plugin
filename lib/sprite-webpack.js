...
function getStyles() {
  ...
  function cssTemplate(list) {
    ...
    _.forEach(list, function(item) {
      var itemName = item.meta.name;
      var isRetina = false;
      if (itemName.indexOf('@2x') !== -1) {
        itemName = itemName.replace('@2x','');
        isRetina = true;
      }

      var classname = itemName;
      var newClassName = item.meta.baseDir + opts.connector + itemName;
      var name = duplicateClassname(classnames, classname, newClassName);
      styles.push({
        'name':     name,
        'x':        isRetina ? item.x / 2 : item.x,
        'y':        isRetina ? item.y / 2 : item.y,
        'width':    isRetina ? item.width / 2 : item.width,
        'height':   isRetina ? item.height / 2 : item.height,
        'isRetina': isRetina ? true : false
      });
